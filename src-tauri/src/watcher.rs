...
impl FileWatcher {
    ...
    pub fn new(db: Arc<db::Database>, watch_state: Arc<Mutex<WatcherState>>) -> Self {
        Self {
            db,
            watch_state,
            watcher: None,
    ...
    pub async fn initialize(&mut self) -> Result<()> {
        ...
        // Clone db and watch_state to avoid moving self into the closure
        let db_clone = self.db.clone();
        let watch_state_clone_closure = self.watch_state.clone();

        let mut debouncer = notify_debouncer_full::new_debouncer(
            std::time::Duration::from_secs(1),
            None,
            move |res: std::result::Result<Vec<notify_debouncer_full::DebouncedEvent>, Vec<notify::Error>>| {
                // This closure is FnMut, so we can call it multiple times
                match res {
                    Ok(events) => {
                        for event in events {
                            ...
                            if matches!(event.kind, notify::event::EventKind::Create(notify::event::CreateKind::File)) {
                                for path in &event.paths {
                                    ...
                                    if detect_screenshot(&path) {
                                        ...
                                        let db_for_task = db_clone.clone(); // Clone again for the spawned task
                                        ...
                                        tokio::spawn(async move {
                                            ...
                                            if verify_file_complete(&path_clone).await {
                                                ...
                                                // Generate thumbnail (includes dimensions)
                                                let (thumbnail, width, height) = match generate_thumbnail(&path_clone) {
                                                    Ok(t) => {
                                                        tracing::info!(
                                                            "Generated thumbnail for {}: {}x{}",
                                                            path_clone.display(),
                                                            t.scaled_dimensions.0,
                                                            t.scaled_dimensions.1
                                                        );
                                                        (Some(t.bytes), t.scaled_dimensions.0 as i32, t.scaled_dimensions.1 as i32)
                                                    },
                                                    Err(e) => {
                                                        tracing::error!(
                                                ...
                                                let filepath = path_clone.to_string_lossy().to_string();

                                                match thumbnail {
                                                    Some(bytes) => {
                                                        match db_for_task.insert_screenshot(
                                                            &filepath,
                                                            filename,
                                                            width,
                                                            height,
                                                            ...
                                                            Err(e) => {
                                                                tracing::error!(
                                                                    "Failed to insert screenshot into database: {}",
                                                                    e
                                                                );
                                                            }
                                                        }
                                                    ...
                                                    None => {
                                                        // Insert without thumbnail if generation failed
                                                        match db_for_task.insert_screenshot(
                                                            &filepath,
                                                            filename,
                                                            width,
                                                            ...
                                                            Ok(id) => {
                                                                tracing::info!(
                                                                    "Ingested screenshot {} with id {} (no thumbnail)",
                                                                    path_clone.display(),
                                                                    id
                                                                );
                                                            },
