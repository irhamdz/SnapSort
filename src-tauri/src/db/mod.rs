    ...
    pub fn get_screenshot(&self, id: i64) -> Result<Option<Screenshot>> {
        ...
        let mut stmt = conn.prepare(
            ...
            SELECT id, filepath, filename, width, height, status,
                   category, category_source, tags, ocr_text, summary,
                   app_detected, is_archived, is_favorite,
        ...
        let row = stmt.query_row(params![id], |row| {
            Ok(Screenshot {
                ...
                thumbnail: None, // Thumbnail fetched separately via get_thumbnail()
...
/// List all screenshots with optional filtering
    pub fn list_screenshots(&self, limit: Option<usize>, offset: Option<usize>) -> Result<Vec<Screenshot>> {
        ...
        let mut query = String::from("SELECT id, filepath, filename, width, height, status, category, category_source, tags, ocr_text, summary, app_detected, is_archived, is_favorite, created_at, updated_at FROM screenshots");
    ...
    pub fn get_thumbnail(&self, id: i64) -> Result<Option<Vec<u8>>> {
        let conn = self.conn.lock().unwrap();

        let mut stmt = conn.prepare(
            "SELECT thumbnail FROM screenshots WHERE id = ?"
        )?;

        let result = stmt.query_row(params![id], |row| {
            row.get::<_, Option<Vec<u8>>>(0)
        }).optional().context("Failed to get thumbnail")?;

        Ok(result.flatten())
    }

    /// Set thumbnail bytes for a screenshot by ID
    ///
    /// This is used when the thumbnail is generated after initial insertion,
    /// or when regenerating a thumbnail.
    pub fn set_thumbnail(&self, id: i64, bytes: Vec<u8>) -> Result<()> {
        let conn = self.conn.lock().unwrap();

        conn.execute("UPDATE screenshots SET thumbnail = ? WHERE id = ?",
            params![bytes, id]
        ).context("Failed to set thumbnail")?;
...
mod tests {
    ...
    #[test]
    fn test_thumbnail_set_and_get() {
        let db = Database::open(test_db_path()).unwrap();

        let id = db.insert_screenshot(
            "/thumb/test.png", "test.png", 1920, 1080, "analyzed",
            "code", "ai", vec![], None, None, 0, 0, 0, None,
        ).unwrap();

        let before = db.get_thumbnail(id).unwrap();
        assert!(before.is_none(), "thumbnail should be null before set");

        let fake_bytes = vec![0xFFu8, 0xD8, 0xFF, 0xE0, 0x01, 0x02, 0x03];
        db.set_thumbnail(id, fake_bytes.clone()).unwrap();

        let after = db.get_thumbnail(id).unwrap();
        assert_eq!(after, Some(fake_bytes), "thumbnail bytes should round-trip");
    }

    #[test]
    fn test_thumbnail_missing_id_returns_none() {
        let db = Database::open(test_db_path()).unwrap();
        let result = db.get_thumbnail(99999).unwrap();
        assert!(result.is_none(), "non-existent id should return None");
    }

    #[test]
    fn test_thumbnail_overwrite() {
        let db = Database::open(test_db_path()).unwrap();

        let id = db.insert_screenshot(
            "/thumb/overwrite.png", "overwrite.png", 800, 600, "analyzed",
            "design", "ai", vec![], None, None, 0, 0, 0, None,
        ).unwrap();

        db.set_thumbnail(id, vec![1, 2, 3]).unwrap();
        db.set_thumbnail(id, vec![9, 8, 7, 6]).unwrap();

        let result = db.get_thumbnail(id).unwrap();
        assert_eq!(result, Some(vec![9, 8, 7, 6]), "second set should overwrite first");
    }
}